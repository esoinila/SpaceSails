...
- Time fade (added post-v1, owner ask): on ghost paths only, per-segment alpha as a function of
  |sample time − scrub time| — past decays to a low floor, far future to a higher one —
  quantized into a dozen alpha buckets so each path still strokes in a few runs. Non-ghost
  families (sweep fans) stay flat: they are alternate flights, not a timeline. Toggleable;
  candidate for export to the game's own trajectory ribbons.
