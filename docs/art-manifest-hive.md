...
## #853 — the posters of the eternally promising

Owner, 2026-08-12, a postdoc's own gag:

> *"as gags we could have gen-ai conference posters ... jokes about how hydrogen is the new promising tech
> (still 100 years in future), etc :-D ... you know those promising technology of the future (and always
> will be) :-D"*

…and on the pool as written: *"yes love those :-D"* — **the eight are canonical.** The copy lives in
`LabPosters.Pool` and is guarded word for word by
`NeverAnEmptyFloorTests.ThePosterPoolIsTheOwnerBlessedCopy`.

**The pictures may not repeat the copy.** The house rule for this whole set is *no text, no lettering, no
numbers*, and these are the one set where that rule is load-bearing rather than tidy: every word of the joke
is written in code and shown on the [E] card, and a poster whose pixels also shouted HYDROGEN would say it
twice and be unfixable the day a word changes.

- **Slots:** `LabPosters.ArtUrlFor(1..8)` → `art/lab-poster-<slug>.jpg`, drawn on the ViewObject card
  `HiveInterior` hangs beside each laboratories chamber's doorway. Degrades cleanly like every slot in this
  file: no file, no frame, and the fine print still reads.
- **One style pass for the set.** They are eight prints of the same species, seen down the same corridor: an
  A0 portrait sheet in a thin aluminium frame under polycarbonate, bolted flat to shotcrete, lit by the
  corridor's own strip. Faded four-colour offset going magenta, a soft crease where it was once rolled, dust
  in the frame's bottom rail. **Blocked-out abstract poster layout** — a title band, a hero panel, a grid of
  small figure boxes, a footer strip — with **no readable characters anywhere**. It reads as an academic
  conference poster and says nothing.
- **Canon-bound** (house law §0, and #853's own): a poster may show a diagram, a lattice, a cutaway vessel, a
  curve going up. It may never show what this facility produced. #8 in particular must be the blandest of the
  eight — a plain title band and one flat schematic panel. Its horror is that it is on *this* wall, and the
  picture must not be in on the joke.

| # | Slot | The hero panel |
|---|------|----------------|
| 1 | `art/lab-poster-hydrogen.jpg` | a cutaway pressure vessel and a rising curve, drawn with the confidence of 1974 |
| 2 | `art/lab-poster-fusion.jpg` | a torus in cross-section, and **a small pale rectangle stuck over one corner of the sheet** with the edges of three older ones under it |
| 3 | `art/lab-poster-superconductivity.jpg` | a lattice diagram, and the whole sheet yellowed a full step beyond the other seven |
| 4 | `art/lab-poster-space-elevator.jpg` | a tapering ribbon into cloud, rendered as an investment prospectus: generous white space, one restrained accent colour |
| 5 | `art/lab-poster-thorium.jpg` | a fuel-cycle flow diagram of boxes and arrows, cheap two-colour print |
| 6 | `art/lab-poster-solid-state.jpg` | an exploded cell stack, with typographic furniture visibly a generation older than the other seven |
| 7 | `art/lab-poster-alignment.jpg` | a block diagram with a feedback loop, and **a diagonal paper band across one corner** (the postponement, unreadable) |
| 8 | `art/lab-poster-neural-state.jpg` | one flat schematic panel and nothing else — the plainest sheet on the wall, hung a few degrees off square |

- **Not generated.** Slots published 2026-08-12 with the copy; the pictures are a later gen-AI pass. Until
  then the card is caption-only, which is the shipped degradation path and not a gap.
