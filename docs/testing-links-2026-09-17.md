...
| **…and the price is what rotates** (#1272) | [`?ashore=1&dock=the-space-bar&simhours=1`](https://esoinila.github.io/SpaceSails-play/map?ashore=1&dock=the-space-bar&simhours=1), then `&simhours=5`, then `&simhours=9` — a watch is **4 sim-hours**, so each of those is a fresh shift. **THE ROADSTEAD, and not the Ringside the three rows above stand at** (#1272): the Ringside honestly chalks 3 cr for four shifts running — four values and a short window will do that — so a tester who followed this row saw the number sit still and had every reason to file *"the rotation is dead"*. | The **dish does not change** — there is one Special per bar and the game does not pretend otherwise — but the **number on the button does: `5 cr → 6 cr → 4 cr`**, in that order, inside 3–6 cr, and it is the same number every time you boot that same watch. Those three numbers are `TheMenuBoard.PriceOn`'s own, and `TheBoardIsNotThePourTests` re-derives them from the rule and reds if this row ever names a bar and three watches that do not move. | — not driven |
