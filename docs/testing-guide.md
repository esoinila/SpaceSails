...
| **`?secretlab=sealed`** | **Park the one rock whose site carries THE REFUGE THAT FAILED (#619); ride to it with `&land=1&floor=3`.** The welded room is on **one floor of one site in four**, and none of the other three cheat rocks happens to have one — so without this the beat is unreachable from a URL, which is the sentence `?secretlab=deep` was added for one feature along. It overrides no Core fact: a site's whole shape is seeded off its body id (`secret-lab-site-sealed`), so what you walk is what a captain would walk, and `TheRefugeThatFailedTests` pins that the story is still on **B3 · LONG STORAGE** so the link cannot quietly start landing in an ordinary building. **What a tester should see on B3:** two `🫁 REFUGE` plates at two tones, two rings on the fan that do not look alike (one calm and breathing, one grey, hollow and still), the legend `🫁 refuge · dark` under the tracker readout, and a welded leaf with a real wall behind it that no sentry can shoot open — `[E]` there raises **🫁 THE REFUGE THAT FAILED** once, with its painting, and files one line under the floor. Then walk one room over: the floor's OTHER refuge cycles and fills your tank, which is the whole law. Full walkthrough in `docs/testing-links-the-hive.md` §8. |
...
| **`?havenfloor=N`** | **…AND THEN ONE FLOOR DOWN (#1253).** Owner, 2026-09-20: *"could we add a basement level to the observation deck station, so the tailing task could start from the basement cabin and end at the observation deck? Otherwise the followed distance is easily very short. The main hall could have multiple elevators… good for tailing."* **Selene Gate** is the one station with a floor under its concourse — a **LOWER CONCOURSE**: a service corridor running the whole way round under the hall, a row of **five crew cabins** whose leaves do not open for a captain (press one; it is time, not a key — #563), worklight, and **three cages** back up. The cars stand on three different edges of the twelve-gon and **each lands at its own edge**, which is the whole of the tailing craft this cheat exists to make playable: which car did he take, and where did it put him. `?havenfloor=-1` IMPLIES `?ashore=1` and defaults `&dock=selene-gate`; `?havenfloor=0` is the concourse, which is where every boot already starts. A level this game does not have is not read at all, so a typo is a concourse and never a building at level −4. **What a tester should see:** you are standing where the first cage's doors open, with `🛗 LIFT · L-00` at your elbow; the deck's location strip reads **LOWER CONCOURSE**; `[E]` at a cabin plate is refused, with no card and no key anywhere in the game that opens one; `[E]` at a car opens the same **🛗 CAR PANEL** the Hive's lift draws, with **CONCOURSE** and **SERVICE LEVEL** on it and *You are at SERVICE LEVEL* under the title. Ride up on a **different** car and you come out at a different edge of the hall. **Broken looks like:** the panel offering only one floor (that is #600 in a new coat); the captain coming out inside a wall; anybody from the bar walking about down there. *(Also a button in the front door's **⚙ DEV START SITES** list — 🛗🌑 “Selene Gate — the service level”.)* |
...
| **`?parkphase=night\|dawn\|day\|dusk\|morning`** | **THE PARK'S OWN HOUR (#759), with `?park=1`** — jumps the sim clock so **this site's** park is at a chosen point of *its own* grow-cycle when the captain arrives. It is `?simhours=` with the arithmetic done for you, and it has to be: the cycle carries a **per-site phase offset**, so one sim-time is this park's afternoon and the next park's dark. The park runs a photoperiod of **4.618034 watches — 18 h 28 m**, four watches and the *golden section* of a fifth, which is the worst-approximable ratio there is; its morning therefore walks around the building's four-hour watch for ever and never settles on it (measured: the two clocks disagree on 80% of watches — a majority, and not all). What a tester should see: on `night` the gravel is drawn at 55% of its noon alpha — dim, never unreadable — and the five floodlight masts are **small dark heads**; on `day` each is a **wash of cold horticultural white**, deliberately not the warm amber every other light on this deck wears; `dawn` and `dusk` are the shoulders, straight 55-minute ramps rather than a switch. **Nothing in any HUD, clock, label, plate or card ever says what time it is in there** — a source sweep holds it, and the player can only see it. `morning` is the row that lands the BEAT: it sets you down **five sim-minutes** before this park's own dawn, on a cycle the rest of the building is *not* having a morning on. Stand still (or take a bench — it makes no difference which) and one ranked pulse lands, once per captain per site, with one lower-case line in the field book filed under the PLACE. Walk out of the gate and back in first and **nothing** lands: you came in again, and the beat is the change. *(Also two buttons in the front door's **⚙ DEV START SITES** list — 🌳🌗 "The park at an hour of its own" and 🌳🌅 "…and standing in it when the morning comes up".)* |
...
| **`?special=story`** | **FORCE THE BOARD'S RARE OUTCOME (#247).** Every haven bar chalks a **FOOD BOARD** beside its drinks card now, with one **TODAY'S SPECIAL** on it — *"SPECIAL."* at the Roadstead and the keep will not elaborate; hydroponic eel at the Ringside, *"caught it ourselves, don't ask where"*. Ordering it is a **meal-sized shore-leave beat**: small coin (3–6 cr, re-chalked every watch), a bigger ease of the nerve than a tot, **one Special a sitting**, and a **d20** for what lands. Usually *"It is better than it had any right to be."* Occasionally — one plate in ten — *"Something in it moved, and then it was delicious."* That roll is seeded on the **captain** as well as the bar and the watch, which is the right design and also means **no URL can show you the rare line**: every universe rolls its own. So this forces the **ROLL** and nothing else — `?roll=`'s own philosophy (#746) and `?tender=flash`'s (#1022): the dish is still this bar's dish, the price is still this watch's price, the purse still moves once, one-Special-a-sitting still holds, and the face printed on the receipt is the **real** face the dice cast. **What a tester should see:** on `/map?ashore=1&dock=ringside-exchange&special=story`, press `[E]` at the counter and then **🍽 TODAY'S SPECIAL** on the green-ruled board above the buttons — the purse drops by the chalked price and the receipt reads the dish, then *"Something in it moved, and then it was delicious."*, then the d20 face, then the steadying note. Press it again the same visit and the keep says *"One Special a sitting, spacer"* and takes nothing. Drop `&special=story` and the same plate comes up *"better than it had any right to be"* instead. **Broken looks like:** the purse moving twice; the deck going tilty off a bowl of noodles (food is not a pour — #756); the receipt naming rum; the board missing at a bar; or the rare line arriving without `&special=story` more often than about one plate in ten. |
...
| `/map?tailed=1` | **#1062 slice 2 · SOMEBODY IS BEHIND YOU.** Ashore with a man following the captain, whatever the outfit's folder says (`?tailed=0` keeps the concourse empty). He comes in after you through the bar's own door, keeps nine to thirty deck units, stands where he can see you, **orders nothing and wears no name** — and until you find him **the game says not one word**. Two ways to find him: take a top with a clear line to the doorway and sit for about nine seconds (*"From this chair you can see the door…"*), or double back so he comes through two distinct doorways (*"The same grey coat, two doors running…"*). To be rid of him, break his line for about nine seconds — walking back down your own gangway does it, because he will not follow you onto your ship. Best berth is `&dock=selene-gate`, where #1199's observation walk is a one-way room he has to come to the mouth of. Full note: `docs/features/losing-the-tail.md`. |
...
/map?park=1&parkphase=night            …the same park at the bottom of ITS OWN grow-cycle (#759): dim gravel, five dark masts
/map?park=1&parkphase=morning          …and standing in it five sim-minutes before its morning comes up, at nobody else's hour
