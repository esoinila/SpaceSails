...
public sealed class TheShelfIsReadWhereItStandsTests
{
    ...
    /// four partials now and the claims below are about the ORDER of three indices in it, so the text has to
    /// be the whole surface in the order the one file laid it out. Every other path this helper is handed
    /// still comes back as <c>File.ReadAllText</c>.</summary>
    ...
    /// <summary>#870 · The surface page is thirty-one partials by subject now, so "the surface wiring" a guard
