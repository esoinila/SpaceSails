...
public sealed class TheGroundHasItsOwnVoiceTests
{
    ...
    {
        var outside = HullShudder.LinesFor(HullShudder.Setting.Regolith, HullShudder.HavenRoom.Concourse);
        var inside = HullShudder.LinesFor(HullShudder.Setting.DeepSite, HullShudder.HavenRoom.Concourse);
    ...
    public void NoRegolithLineMentionsAHullARoomOrACrowd()
    {
        ...
        foreach (string line in HullShudder.LinesFor(HullShudder.Setting.Regolith, HullShudder.HavenRoom.Concourse))
        {
            foreach (string bad in forbidden)
            {
