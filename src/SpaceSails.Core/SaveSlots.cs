...
public sealed record SaveSlotMeta
{
    ...
    // ── #948 · The human half of the row. Owner, looking at eight identical berths: "Let's have an option to
    //    change the name of our avatar. It solves the issue of forgetting who you are amongst the autogenerated
    //    names", and "I would like to save this point with a comment field and a captain's name of my whim."
    //    Additive manifest data (the JSON is tolerant): a slot banked before #948 reads all three back as ""
    //    and the row falls back to the seeded captain and DefaultTitle. Mirrored from the payload's own
    //    LogbookSection so the row and the exported file can never disagree.

    /// <summary>The captain's name at the moment of saving — the renamed one, or the seeded roster name.
    /// Blank on a pre-#948 slot; the caller re-derives the seeded name from the thread id.</summary>
    public string CaptainName { get; init; } = "";

    /// <summary>The captain's-whim title for this moment. Blank = untitled; the row shows
    /// <see cref="SaveSlotLabels.DefaultTitle"/> instead.</summary>
    public string Title { get; init; } = "";

    /// <summary>What the captain was doing when they kept this moment. Free text, never parsed.</summary>
...
public static class SaveSlotLabels
{
    ...
    public static SaveSlotMeta PreviewMeta(Vault? vault) => new()
    {
        ...
        // #948 — a file names its own captain and carries its own page, so an import preview can say WHOSE
        // moment it is before the captain commits to boarding it. A pre-#948 file has no logbook: blanks.
        CaptainName = CleanName(vault?.Logbook?.CaptainName),
        Title = CleanTitle(vault?.Logbook?.Title),
        Note = CleanNote(vault?.Logbook?.Note),
    };

    // ── #948 · titles and notes: the one place their shape is decided. ──

    /// <summary>How long a captain's-whim title may be before the row stops being a row.</summary>
    public const int MaxTitleLength = 80;

    /// <summary>How long a note may be. Generous — it is a paragraph about what you were doing, not a
    /// tweet — but bounded, because it rides in localStorage beside nine other slots.</summary>
    public const int MaxNoteLength = 600;

    /// <summary>Tidy a typed title: trim, collapse every run of whitespace (newlines included — a title is
    /// ONE line), and cut to <see cref="MaxTitleLength"/>. Null/blank comes back "" (= untitled).</summary>
    public static string CleanTitle(string? title) => Tidy(title, MaxTitleLength, oneLine: true);

    /// <summary>Tidy a typed note: trim and cut to <see cref="MaxNoteLength"/>, but KEEP the line breaks —
    /// a note is allowed to be a paragraph. Null/blank comes back "".</summary>
    public static string CleanNote(string? note) => Tidy(note, MaxNoteLength, oneLine: false);

    /// <summary>Tidy a captain's name the same way a title is tidied, at the roster's own length.</summary>
    public static string CleanName(string? name) => Tidy(name, Captains.MaxNameLength, oneLine: true);

    private static string Tidy(string? text, int max, bool oneLine)
    {
    ...
    /// <summary>The title a save gets when nobody typed one: <c>&lt;captain&gt; — &lt;place&gt; — day N</c>
    /// (the default the bank dialog offers, and the fallback a row shows for an untitled slot). Every part is
    /// optional — a nameless captain, an unknown place and a day zero all still make a readable line.</summary>
    public static string DefaultTitle(string? captainName, string? where, int simDay)
    {
        string captain = CleanName(captainName);
        string place = string.IsNullOrWhiteSpace(where) ? "unknown waters" : where.Trim();
        string day = $"day {Math.Max(0, simDay)}";
        string head = captain.Length > 0 ? $"{captain} — {place}" : place;
        return CleanTitle($"{head} — {day}");
    }

    /// <summary>The title to PRINT on a slot row: the captain's own, when they gave one; otherwise the
    /// derived <see cref="DefaultTitle"/> built from that slot's own captain, place and day. One truth, so
    /// the drawer, the front door and the export filename cannot drift apart.</summary>
    public static string TitleOf(SaveSlotMeta meta)
    {
        ArgumentNullException.ThrowIfNull(meta);
        string given = CleanTitle(meta.Title);
        return given.Length > 0 ? given : DefaultTitle(meta.CaptainName, meta.Where, meta.SimDay);
    }
}
