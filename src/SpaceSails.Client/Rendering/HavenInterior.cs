...
/// round immigration hall + bar, 2026-07-08). A client-side <c>bodyId → </c><see cref="DeckPlan"/>.
///
/// The Expanse model (owner): docking mates the ship's airlock to the station by a <b>narrow
/// umbilical with automatic doors</b>, and you <b>walk</b> across — no teleport. And the station is
/// much bigger than the ship: a small-airport-sized structure. So a docked haven welds, in one
/// coordinate space: the ship (airlock a defensible port vestibule) → the tube → a <b>big round
/// entrance hall</b> (a 12-sided ring — <b>10 other berths' hatches sealed</b>, so it reads like a
/// station with a dozen ships docked; a Mars-immigration desk, à la Total Recall; signage) → a wide
/// door → the <b>bar</b>, tables you walk up to. The top-down view follows you
/// (<see cref="DeckPlan.FollowCam"/>); the welded deck is ~80 du long.
///
/// First up: <b>The Rusty Roadstead</b> off Mars — rust-red planet and Phobos out the hall glass.
/// </summary>
public static class HavenInterior
{
    ...
    public static bool HasInterior(string bodyId) => bodyId == "the-space-bar";

    /// <summary>The docked complex for a body — ship + tube + hall + bar as one walkable plan — or
    /// null if that haven has no deck to walk (yet). Built once, lazily, and shared.</summary>
    public static DeckPlan? DockedDeck(string bodyId) => bodyId switch
    {
        "the-space-bar" => RustyRoadstead.Value,
        _ => null,
    };
    ...
    // --- The bar, off the hall's north door — big and cavernous, a red-planet view along the back ---
    private const float BarLeft = -14f;
    private const float BarRight = 19f;
    private static readonly float BarTopY = HallTopY + 22f;

    private static class RustyRoadstead
    {
        ...
        private static DeckPlan Build()
        {
            ...
            // Mars-immigration desk (Total Recall): two counters with a central GATE aligned to the
            // tube, so you walk straight off the umbilical through the checkpoint. Officer to one side.
            ...
            labels.Add((HallCenterX, HallBottomY + 7.5f, "MARS IMMIGRATION"));
            labels.Add((HallCenterX, HallBottomY + 2.5f, "most guests stay two weeks"));
            labels.Add((HallCenterX, HallCenterY + 3, "⚓ THE RUSTY ROADSTEAD"));
            ...
            labels.Add((HallCenterX, BarTopY - 3, "THE ROADSTEAD BAR"));
            ...
            [
                .. ship.Backdrops,
                // Lobby art inscribed in the round hall (square within the 12-gon so it doesn't bleed past the walls).
                new("art/the-rusty-roadstead-lobby.jpg", HallCenterX - 12, HallCenterY + 12, 24, 24, 0.95f),
                new("art/the-roadstead-bar.jpg", BarLeft, BarTopY, BarRight - BarLeft, BarTopY - HallTopY, 0.95f),
            ];
            ...
            return new DeckPlan(walls.ToArray(), consoles.ToArray(), labels.ToArray(), backdrops,
                ...
                location: static (x, y) => y > HallTopY ? "THE ROADSTEAD BAR"
                                         : y > HallBottomY ? "MARS IMMIGRATION"
                                         : y > ShipHatchY ? "GANGWAY"
                                         : DeckPlan.Ship.Location(x, y),
                doors: doors.ToArray(), shipFixtures: true, followCam: true, tables: tables.ToArray());
        }

        // Ship's three droids, the immigration officer, and the three seated bar regulars.
