...
// GroundGrewCard — the code-behind for GroundGrewCard.razor.
//
...
//
// Nothing below was rewritten: it is the `@code` block of GroundGrewCard.razor, character for character, in the
...
public partial class GroundGrewCard
{
    ...
    // #584 · WHERE the ground grew, composed by GroundGrows.Where out of the floor's own plate, the SDR
    // compass and the fan's own "N du". It keeps the page field's own name, underscore and all, per the
    // note above — this surface renders the page's member, it does not own a second copy of the fact.
    [Parameter] public string _groundGrewWhere { get; set; } = "";
