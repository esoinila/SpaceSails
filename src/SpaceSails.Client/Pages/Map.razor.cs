...
//
// ── #1107 · FOUND, AND NOW FIXED ─────────────────────────────────────────────────────────────────────
// Moving these two methods surfaced two CA2014s — "Potential stack overflow. Move the stackalloc out of
// the loop." — which were the whole point of the exercise: both had been in the frame loop for months and
// neither had ever been shown to anybody, because they were written inside `@code`. The stack a
// `stackalloc` takes is not released at the end of an iteration; it is held until the METHOD returns, so a
// `stackalloc` in a loop grows the frame once per pass. DrawPyramids' loop is bounded by
// `AncientsRule.PyramidCount` and is small; DrawOrdnance' is bounded by the number of live rounds, which
// nothing in the sim caps — a long fight is a frame that keeps taking stack it will not give back until it
// returns.
//
// #1140 suppressed them at the site rather than fix them, because that lane's contract was pure motion and
// a suppression that says so out loud is how a finding survives a move instead of being laundered by it.
// This is the lane that takes the fix, and it is one line each: the buffer is declared ONCE above the loop
// and reused. Behaviour-identical by inspection — every element of both buffers is written before it is
// read on every pass (all eight of `triangle`, all four of `streak`), so no iteration can observe what a
// previous one left in it. The pragmas go with the findings, so CA2014 is live at both sites again.
//
// The proof is the ledgers: FrameHashes.ledger.txt fingerprints what the renderer is actually TOLD to
// draw, and it does not move. A moved frame hash here would mean the pen changed — not that the ledger
// needed re-pinning.
public partial class Map
{
    private void DrawPyramids()
    {
        // One buffer for the whole method, refilled per pyramid: all eight elements are written below
        // before DrawPolyline reads any of them, so a pass never sees the last one's corners.
    ...
    {
        // Same as above, and this is the one that mattered: nothing in the sim caps how many rounds are in
        // the air, so this loop's stack cost was unbounded. All four elements are written per pass.
